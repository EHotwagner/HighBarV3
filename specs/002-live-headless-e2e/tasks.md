...
- [ ] T044 [US5] Create BAR Lua test widgets under `tests/headless/widgets/` — widgets covering the `MapDrawCmd`, `DrawInMinimap`, `DrawWorldPreUnit`, and `GroupChanged` callins required by the 11 Channel C arms (contracts/aicommand-arm-map.md Channel C). Each widget records call records retrievable via `InvokeCallback` under a well-known name. Update `tests/headless/widgets/README.md` listing every file. **subsumed-by-003 (Channel-A+B subset)**: [specs/003-snapshot-arm-coverage/](../003-snapshot-arm-coverage/tasks.md) T024 registers Channel-C arms with the `NotWireObservable` sentinel and records them in the macro coverage CSV; the Lua-widget follow-up remains deferred per spec §Out of Scope.
- [ ] T045 [US5] Create `tests/headless/aicommand-arm-coverage.sh` — iterates all 66 arms (reading from `build/reports/aicommand-arm-coverage.csv`), for each arm dispatches a single batch from the Python AI-role client against a live match, asserts the arm's declared observability channel fires (state-stream diff for Channel A, engine-log grep for Channel B, widget-record retrieval for Channel C). Exit 0 iff every arm's assertion passes. Source `_fault-assert.sh` + `_launch.sh` (FR-013). **subsumed-by-003**: [specs/003-snapshot-arm-coverage/](../003-snapshot-arm-coverage/tasks.md) T029 (aicommand-behavioral-coverage.sh) is the behavioral-verification equivalent; the wire-coverage CSV from 002's `_gen-arm-coverage.sh` continues to track dispatcher wiring separately.
...
- [X] T065 [US7] Add assertion in damage-path acceptance scripts — specifically `tests/headless/us1-observer.sh`, `us2-ai-coexist.sh`, `tests/bench/latency-uds.sh`, `tests/bench/latency-tcp.sh`, plus `tests/headless/aicommand-arm-coverage.sh` for any arm that can produce a `UnitDamaged` delta — that `damage > 0.0` for all observed damage events and at least one component of `direction` is non-zero for checkable events with `attacker_id` present, proving the widening works end-to-end without inventing direction for unattributed engine events (contracts/unit-damaged-payload.md §Test assertions).
...
- [X] T066 [P] Create `tests/headless/gateway-fault.sh` — deliberately triggers an internal fault and asserts the disable contract: (a) engine log contains `[hb-gateway] fault subsystem=… reason=…`, (b) `$writeDir/highbar.health` contains `"status":"disabled"`, (c) the coordinator observes the push stream close, (d) heartbeats stop (FR-024, contracts/gateway-fault.md). A malformed client payload rejected with `INVALID_ARGUMENT` is a valid `77` skip for this harness and is covered separately by a malformed-payload resilience test.
...
- [ ] T068 Run the full acceptance suite on the reference host five consecutive times; verify all 17 headless scripts + both bench scripts + `ctest` exit 0 every time (SC-002 reproducibility). Record results. **subsumed-by-003**: [specs/003-snapshot-arm-coverage/](../003-snapshot-arm-coverage/tasks.md) T032 (behavioral-reproducibility.sh) runs the macro driver 5× and asserts byte-identical digests — the reproducibility signal that T068 targets.
- [ ] T069 Run the framerate reproducibility pass five times; verify p50 framerates stay within ±5% run-to-run (FR-008). Record in `specs/002-live-headless-e2e/research.md` §R9 or a new results appendix. **subsumed-by-003**: T032 already asserts `(max - min) / median ≤ 0.05` across the 5 macro runs' p50 framerates.
